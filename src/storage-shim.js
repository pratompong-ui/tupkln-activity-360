...
async function loadSharedData() {
  const [settingsRes, unitsRes, actsRes, docsRes] = await Promise.all([
    ...
    supabase.from('activity360_docs').select('*'),
  ]);
  ...
  return {
    ...
    units,
    activities,
    // ตาราง activity360_docs ยังไม่มีก็ไม่เป็นไร ระบบจะทำงานต่อโดยไม่มีหนังสือ
    docs: docsRes.error ? [] : (docsRes.data || []).map((d) => ({ id: d.id, ...(d.payload || {}) })),
  };
}
...
async function persistSharedData(next) {
  ...
  if (activities.length) {
    ...
    if (error && /activity_end_date|doc_url|category|album_url|join_count|tasks|column|schema cache/i.test(error.message || '')) {
  ...
  // หนังสือเข้า เก็บทั้งก้อนเป็น jsonb เพื่อให้เพิ่มฟิลด์ใหม่ได้โดยไม่ต้องแก้ตาราง
  try {
    const docs = (next.docs || []).map((d) => {
      const { id, ...rest } = d;
      return { id, payload: rest, updated_by: uid };
    });
    const { data: existingDocs } = await supabase.from('activity360_docs').select('id');
    const keep = new Set(docs.map((d) => d.id));
    const drop = (existingDocs || []).map((x) => x.id).filter((id) => !keep.has(id));
    if (drop.length) await supabase.from('activity360_docs').delete().in('id', drop);
    if (docs.length) {
      const { error } = await supabase.from('activity360_docs').upsert(docs);
      if (error) throw error;
    }
  } catch (err) {
    console.warn('บันทึกหนังสือเข้าไม่สำเร็จ ตรวจว่ารันสคริปต์ add-docs.sql แล้วหรือยัง:', err);
  }
